...
import random
from config import INIT_TRAINER, CHANNEL, INPUT_SHAPE,SETUP_TRAINER, CURRENT_FOLD, PATH_LIST, FOLD_NUM,TEST_PATH,MODE
...
if __name__ == "__main__":
    ...
    if args.mode != 'train-cross':
        ...
        count_params_and_macs(segnetwork.net.cuda(),(1,CHANNEL) + INPUT_SHAPE)
    path_list = PATH_LIST #数据集路径
    # Training
    ###############################################
    if args.mode == 'train-cross':
    ...
    if args.mode == 'train':
        ...
        SETUP_TRAINER['train_path'] = train_path
        SETUP_TRAINER['val_path'] = val_path
        SETUP_TRAINER['cur_fold'] = CURRENT_FOLD #当前是第几个文件夹，默认为5
    ...
    ###############################################
        ...
        test_path = r'E:\PI-CAI22\test_2d_seg'
        for current_fold in range(1, 6):
            print("=== Predicting Fold ", current_fold, " ===")
            CKPT_PATH = './new_ckpt/{}/{}/fold{}'.format(MODE,str(current_fold))
            INIT_TRAINER['weight_path'] = r'ckpt\PI-CAI22\2d.pth'
            segnetwork = SemanticSeg(**INIT_TRAINER)
            save_path = './segout/2d/fold{}'.format(current_fold)
