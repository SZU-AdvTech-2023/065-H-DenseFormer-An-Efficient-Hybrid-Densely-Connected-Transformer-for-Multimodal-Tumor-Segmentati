...
from loss.combine_loss import get_masked_ptc_loss,label_to_aff_mask,cam_to_label
...
class SemanticSeg(object):
  ...
  '''
    ...
    def trainer(self,
        ...
        net = self.net
        lr = self.lr
        loss = self._get_loss(loss_fun, class_weight) #FocalLoss
        ...
        for epoch in range(self.start_epoch, self.n_epoch):
            ...
            self._train_on_epoch(net, loss, optimizer, train_loader, scaler)

            val_loss,  val_run_dice = self._val_on_epoch(
            ...
            print('epoch:{}/{},,val_loss:{:.5f}'.format(
                epoch, self.n_epoch,  val_loss))

            print(
                'epoch:{}/{},val_run_dice:{:.5f}'
                .format(epoch, self.n_epoch,  val_run_dice))
            ...
            if val_run_dice > self.metrics_threshold:
                ...
                file_name = 'epoch={}-val_loss={:.5f}-val_run_dice={:.5f}.pth'.format(
                    epoch,  val_loss, val_run_dice)
    ...
    def _train_on_epoch(self , net, criterion, optimizer, train_loader,
                        scaler):
        net.train()
        ...
        for sample in train_loader:
            ...
            sort_li=sorted(list(net.named_parameters()),key=lambda x:x[1].numel(),reverse=True)[:3] #找参数量最大的三个模块
            with autocast(self.use_fp16):
                # output,fmap,cam_aux = net(data)
                output = net(data)
            loss = criterion(output, target)
            # pseudo_label_aux = cam_to_label(cam_aux.detach(), ignore_mid=True, bkg_thre=0.5, high_thre=0.7,
            #                                     low_thre=0.25, ignore_index=255)
