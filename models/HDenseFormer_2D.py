...
class Dense_TransformerBlock(nn.Module):
    ...
    def forward(self, img):
        ...
        for block, in self.blocks:
            x = block(x)
            if i==self.depth/2:aux=x
            i+=1
        x = self.re_patch_embedding(x)
        aux = self.re_patch_embedding(aux)
        return F.interpolate(x, self.outsize),F.interpolate(aux, self.outsize)
...
class HDenseFormer_2D(nn.Module):
    def __init__(self, in_channels, n_cls, n_filters, image_size=(384,384), transformer_depth=12):
        ...
        self.block_4_2_left = BasicConv2d(8 * n_filters, 8 * n_filters, kernel_size=3, stride=1, padding=1) #卷积
        ...
        self.aux_classifier = nn.Conv2d(in_channels=384, out_channels=self.n_cls-1, kernel_size=1, bias=False,)
    ...
    def forward(self, x):
        # inputs_cat = torch.cat([x, x.flip(-1)], dim=0)#flip：对n维张量的指定维度进行反转
        att,aux=[],[]
        for i in range(self.in_channels):
            att.append(self.attns[i](x[:,i:i+1,:,:])[0])
            # aux.append(self.attns[i](inputs_cat[:,i:i+1,:,:])[1])
        attnall = torch.cat(att,1)
        # aux = torch.cat(aux,1)
        # cam_aux = F.conv2d(aux, self.aux_classifier.weight).detach()

        # b, c, h, w = x.shape
        # _cam_aux = torch.max(cam_aux[:b,...], cam_aux[b:,...].flip(-1))
        ...
        return [x,out1,out2,out3]#,attnall,cam_aux
